...
public class UserResource extends BaseObjectResource<User> {
   ...
   @GET
     @Path("/{id}")
    public Collection<User> get(@QueryParam("userId") long userId,@PathParam("id") String id) throws SQLException {
        UsersManager usersManager = Context.getUsersManager();
        Long result = -1l;//no hay id -1
        if (Context.getPermissionsManager().getUserAdmin(getUserId())) {
            if (userId == 0) {
        ...
        return usersManager.getItem(result);
    }
